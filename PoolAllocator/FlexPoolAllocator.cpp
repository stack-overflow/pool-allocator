...
FlexPoolAllocator::FlexPoolAllocator() :
    m_default_num_elements(128),
    m_last_used_allocator(std::pair<size_t, AllocationEngine*>(0, NULL))
{}

FlexPoolAllocator::FlexPoolAllocator(size_t num_elements_per_pool) :
...
AllocationEngine* FlexPoolAllocator::get_allocator(size_t byte_size)
{
...
void *FlexPoolAllocator::allocate(size_t byte_size)
{
...
void FlexPoolAllocator::deallocate(void *p, size_t byte_size)
{
    if (m_last_used_allocator.first == byte_size)
    {
